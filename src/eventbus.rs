...
impl<'b> Eventbus<'b> {
    ...
    pub fn connectFuture(addr: &str) -> impl Future<Item=Eventbus<'static>, Error=Error> {
        let connectFuture = TcpStream::connect(&addr.parse::<SocketAddr>().unwrap());
        connectFuture.map(|tcpstream| {
            ...
            let mut stream_clone = tcpstream.try_clone().expect("clone failed");
    ...
    pub fn connect(addr: &str)->Result<Eventbus<'static>,Error>{
        let mut a=futures::executor::spawn(Eventbus::connectFuture(addr));
        a.wait_future()
    }
